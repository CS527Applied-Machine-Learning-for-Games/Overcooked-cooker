                        ...
                        if (abs(dx) == 2 or abs(dz) == 2) and (cur_state["pos"][1][0] + dx//2, cur_state["pos"][1][1] + dz//2) not in invalid_cells[level]:
                            ...
                            if inferred_pos not in invalid_cells[level]:
                                miss_state["pos"][1] = [inferred_pos[0],
                                                        inferred_pos[1],
                                                        action2face[dir2action[(inferred_pos[0] - cur_state["pos"][1][0],
                                                                                inferred_pos[1] - cur_state["pos"][1][1])]]]
                                miss_action = dir2action[inferred_lastmove]
                                action = dir2action[(inferred_pos[0] - cur_state["pos"][1][0], inferred_pos[1] - cur_state["pos"][1][1])]
                            else:
...
if __name__ == '__main__':
    ...
    b = [0, 1, 2]
    print(a == b)
    infer_acton("../data/test1.json", "../data/test_infer.json", "1-2")
