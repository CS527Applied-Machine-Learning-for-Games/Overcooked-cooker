...
class TrainEnv(Env.Env):
    def start(self):
        ...
        with open("../data/1_2.json", "w") as f:
            while True:
                ...
                if (
                    cur_pos != prev_pos
                    or cur_hold != prev_hold
                    or cur_order != prev_order
                    or cur_chopping != prev_chopping
                ):
                    new_state = {
                        "pos": cur_pos,
                        "hold": cur_hold,
                        "order": cur_order,
                        "objects": self.getobjposlist(),
                        "score": self.getscore(),
                        "pot": self.getpotprogress(),
                        "chop": self.getchopprogress(),
                        "fire": self.isfire(),
                    }
