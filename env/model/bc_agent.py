...
def train_model(verbose = False):
    ...
    model.fit(inputs, targets, callbacks = [tensorboard_callback], batch_size = batch_size, 
...
def __test_agent_action():
    agent = BC_Agent()
    print('test agent actin logits: ', agent.action(np.zeros((1, 100)), return_logits = True))
