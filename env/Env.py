...
class Env:
            ...
            for item in templist[itemtype]:
                ...
                if self.__cooradj:
                    ...
                    if abs(xc - item[0]) >= 0.01 or abs(zc - item[1]) >= 0.01:
                        self.__chef_pos = self.pyclient.getchefpos()
                        x = int(
                            (self.__chef_pos[self.__adjchefid][0] - self.__x_border)
                            / self.__step
                        )
                        z = int(
                            (self.__chef_pos[self.__adjchefid][2] - self.__z_border)
                            / self.__step
                        )
    ...
    def getmapcellcenter(self, x, z):
        return [
            (x + 0.5) * self.__step + self.__x_border,
            (z + 0.5) * self.__step + self.__z_border,
        ]
