...
base_map_features = [
    ...
    "B",
    "C",
]  # Table, Work Station, Plate Station, Deliver Station, Ingredient i, Food waste, Cook Station
variable_map_features_dict = {
    "1-2": [
        "player_location",
        "U",
        "R",
        ...
        "plates_with_fish",
        "plates_with_rice",
        "plates_with_rice_seaweed",
        "plates_with_fish_seaweed",
        "plates_with_fish_rice",
...
holding_features_dict = {
    "1-2": ["plate", "pot", "rice", "fish", "seaweed", "fish_chopped"],
    "1-1": ["plate", "fish", "prawn", "fish_chopped", "prawn_chopped"],
}
...
def loss_less_encoding(env, traj_dict=None, encode_holding=True):
    ...
    variable_map_features = variable_map_features_dict[env.map_name]
    ...
    variable_map_layers["player_location"][height - 1 - player_pos[1]][
        player_pos[0]
    ] = 1
    # orientatiOn
    # 0  U, 1  R, 2  D, 3  L
    ...
    for obj in objs:
        ...
        elif obj == "Pot" and env.map_name == "1-2":
            for pos in objs[obj]:
        ...
        # =================== ingredients =======================
            ...
            for pos in objs[obj]:
                if pos in objs["Plate"]:
                    variable_map_layers["plates_with_fish"][height - 1 - pos[1]][
                        pos[0]
                    ] = 1
                else:
                    variable_map_layers["fish"][height - 1 - pos[1]][pos[0]] = 1

        elif obj.startswith("SushiPrawn") and env.map_name == "1-1":
            for pos in objs[obj]:
                if pos in objs["Plate"]:
                    variable_map_layers["plates_with_prawn"][height - 1 - pos[1]][
                        pos[0]
                    ] = 1
                else:
                    variable_map_layers["prawn"][height - 1 - pos[1]][pos[0]] = 1

        elif obj.startswith("Seaweed") and env.map_name == "1-2":
            for pos in objs[obj]:
                if pos in objs["Plate"]:
                    variable_map_layers["plates_with_seaweed"][height - 1 - pos[1]][
                        pos[0]
                    ] = 1
                else:
                    variable_map_layers["seaweed"][height - 1 - pos[1]][pos[0]] = 1

        elif obj.startswith("SushiRice") and env.map_name == "1-2":
            for pos in objs[obj]:
                if pos in objs["Pot"]:
                    ...
                    variable_map_layers["plates_with_rice"][height - 1 - pos[1]][
                        pos[0]
                    ] = 1
                else:
        ...
        elif obj.startswith("ChoppedSushiPrawn") and env.map_name == "1-1":
            for pos in objs[obj]:
                variable_map_layers["prawn_chopped"][height - 1 - pos[1]][pos[0]] = 1

    if env.map_name == "1-2":

        seaweed_rice_fish = (
            variable_map_layers["plates_with_seaweed"]
            * variable_map_layers["plates_with_rice"]
            * variable_map_layers["plates_with_fish"]
        )
...
def get_loss_less_encoding_shape(env=None):
    if env is None:
        ...
        return (
            len(base_map_features)
            + len(variable_map_features_dict[env.map_name])
            + len(holding_features_dict[env.map_name]),
            env.getmapheight(),
            env.getmapwidth(),
        )
